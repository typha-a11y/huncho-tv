...
import { ModalLoadingFallback } from "./ViewLoadingFallback";
...
export function VideoPlayerModal() {
  ...
  useEffect(() => {
    if (selectedMovieId && !isLiveStream) {
      getMovieDetails(selectedMovieId, selectedMediaType).then((data) => {
        if (data) setMovie(data);
      });
    } else {
      setMovie(null);
    }
  }, [selectedMovieId, selectedMediaType, isLiveStream]);
  ...
  return (
    <Suspense fallback={<ModalLoadingFallback />}>
      <StreamPlayerModal
        ...
        mediaType={selectedMediaType || movie?.media_type || "movie"}
      />
    </Suspense>
  );
}
